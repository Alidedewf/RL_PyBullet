...
class RobotArmEnv(gym.Env):
    def __init__(self, render_mode='rgb_array', image_size=64, frame_skip=8):
        ...
        # 1. Пиксели: RGB 64x64x3 (C=3) для MobileNet
        pixel_shape = (image_size, image_size, 3)
    ...
    def _get_observation(self):
        ...
        if img_arr is None or img_arr[2] is None:
             ...
             rgb_img = np.zeros((self.image_size, self.image_size, 3), dtype=np.uint8)
        else:
            # Получаем RGB (без Alpha канала)
            rgb_img = np.reshape(img_arr[2], (self.image_size, self.image_size, 4))[:, :, :3]
            # НЕ конвертируем в Grayscale, оставляем RGB для MobileNet
            
        pixels = rgb_img # Shape: (H, W, 3)

...
class RobotArmEnv(gym.Env):
    def __init__(self, render_mode='rgb_array', image_size=64, frame_skip=8, obs_type='pixels'):
        super(RobotArmEnv, self).__init__()
        
        # --- Параметры Среды ---
        self.image_size = image_size
        self.frame_skip = frame_skip
        self.obs_type = obs_type # 'pixels' or 'coords'
        ...
        self.robotId = p.loadURDF("franka_panda/panda.urdf", useFixedBase=True, basePosition=[0, 0, 0], physicsClientId=self.physicsClient)
        # Получение индекса концевого эффектора (gripper)
        self.end_effector_index = 11
        
        # Загрузка целевого объекта (маленький куб)
        self.targetId = p.loadURDF("cube.urdf", basePosition=[0.5, 0, 0.1], globalScaling=0.08, physicsClientId=self.physicsClient) 
        
        # Получаем информацию о джоинтах
        self.num_joints = p.getNumJoints(self.robotId, physicsClientId=self.physicsClient)
        # panda_joint1 ... panda_joint7 (Indices 0 to 6) for control
        ...
        if self.obs_type == 'pixels':
            ...
            # 1. Координаты цели (x, y, z) - 3 числа
            # 2. Проприоцепция: 7 углов джоинтов Panda
            # Return as a single vectors? Or Dict? Let's use Dict for consistency or Flat Box.
            # Usually MlpPolicy handles passing raw vectors nicely. Let's stick to Box(10,) [7 joints + 3 target]
    ...
    def _get_observation(self):
        # 1. Получение проприоцепции (углы первых 7 джоинтов Panda)
        ...
        if self.obs_type == 'coords':
             ...
             target_pos, _ = p.getBasePositionAndOrientation(self.targetId, physicsClientId=self.physicsClient)
             target_pos = np.array(target_pos, dtype=np.float32)
             
             # Объединяем: [joints(7), target(3)]
             return np.concatenate((joint_positions, target_pos))
    ...
    def step(self, action):
        ...
        obj_pos, _ = p.getBasePositionAndOrientation(self.targetId, physicsClientId=self.physicsClient)
        ...
        contact_points = p.getContactPoints(self.robotId, self.targetId, self.end_effector_index, physicsClientId=self.physicsClient)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Рабочая зона        # 3. Сброс положения цели (случайно на столе)
        rand_x = 0.5 + np.random.uniform(-0.1, 0.1)
        rand_y = 0.0 + np.random.uniform(-0.2, 0.2)
        p.resetBasePositionAndOrientation(self.targetId, [rand_x, rand_y, 0.02], [0, 0, 0, 1], physicsClientId=self.physicsClient)

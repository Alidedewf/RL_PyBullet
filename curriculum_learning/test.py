...
def make_env(render_mode='rgb_array'):
    # obs_type='pixels' по умолчанию, но можно указать явно
    env = DummyVecEnv([lambda: RobotArmEnv(render_mode=render_mode, obs_type='pixels')])
    env = VecFrameStack(env, n_stack=FRAME_STACKS, channels_order='last')
    env = VecTransposeImage(env) # Важно! Neural Style ожидает CHW
    return env
